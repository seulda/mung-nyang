...
export default function Write({ navigation, route }) {
    ...
    const handleAddTask = () => {
        Keyboard.dismiss();        
        console.log('title input: ' + taskTitle);
        console.log('content input: ' + task);
        Alert.alert(`제목: ${taskTitle}\n내용: ${task}`);
    ...
    addCity = (city) => { //state에 저장된 데이터 목록에 새 데이터 추가
        const cities = this.state.cities
        cities.push(city)
        this.setState({cities})
    }
    addLocation = (location, city) => { //선택된 도시에 속한 장소배열에 새 장소를 추가
